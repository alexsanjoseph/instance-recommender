...
def build_pricing_inventory(dest, excluded_regions, raw_inventory_path='utils/instances.json', refresh=False, indent=2):
    if refresh:
        s3 = boto3.client('s3', config=Config(signature_version=UNSIGNED))

        s3.download_file('www.ec2instances.info', 'instances.json', raw_inventory_path)
        ...
        try:
            for region, prices in instance['pricing'].items():
                if region not in excluded_regions:
                    try:
                        final_dict['pricing'][region] = prices['linux']['ondemand']
...
if __name__ == "__main__":
    ...
    argparser.add_argument('--refresh', '-r', action='store_true', default=False, help='Download new inventory')
    args = argparser.parse_args()
    build_pricing_inventory(dest='inventory/instances.json', excluded_regions=['cn-north-1', 'cn-northwest-1'], refresh=args.refresh)
